...
exports.pushPositions = async (e) => {
    ...
    try {
            ...
            if (devPosition.device.attributes.integration) {
                for (const target of devPosition.device.attributes.integration.split(',')) {
                    if (integration[target]) {
                        ...
                        console.warn('no integration for', devPosition.device.attributes.integration, devPosition.device.name, devPosition)
                    }
                }
            } else {
        ...
        console.warn('error processing', deviceId, deviceName, err.message, err.response && err.response.data)
