...
app.post('/optimized-trips', async (req, res) => {
    res.json(await optimize(req.body))
})

app.get('/optimized-trips/:id', async (req, res) => {
    res.json(await get(req.params.id))
})
const googleKey = 'AIzaSyD2CNM15Xqd-WcUpyCZCxynf7Zlikt5-4Y'
const session = 'AJVsH2xVCrU4wIIf5kbZbJmygyOJbYowsGmT4Ormi9rAFpAdwzYbJw8AR83SBUteg45U_QtNagyDYCner0qMHl8BAA'
app.post('/pinmeapi/tile.googleapis.com/v1/createSession', async (req, res) => {
    if (req.body.mapType === 'roadmap') {
        res.json({
            session,
            expiry: '1698863318',
            tileWidth: 256,
...
app.get('/pinmeapi/tile.googleapis.com/v1/2dtiles/:z/:x/:y', async (req, res) => {
    console.log('image',  await image(req, res))
})
